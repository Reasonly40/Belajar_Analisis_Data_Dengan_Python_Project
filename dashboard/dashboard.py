...
if uploaded_file is not None:
    if uploaded_file.name != ALLOWED_FILENAME:
        st.warning(f"Nama file harus '{ALLOWED_FILENAME}'. Silakan unggah file yang benar.")
    else:
        all_df = load_data(uploaded_file)
        
        if 'hour' not in all_df.columns or 'workingday' not in all_df.columns or 'cnt' not in all_df.columns:
            st.error("Dataset tidak memiliki kolom yang diperlukan: 'hour', 'workingday', atau 'cnt'")
        else:
            ...
            ax.legend(title='Hari Kerja', labels=['Tidak', 'Ya'])
            ax.grid(True)
            st.pyplot(fig)

            st.markdown("""
            - Pada hari kerja, peminjaman meningkat tajam pada jam 07.00 - 08.00 pagi dan jam 16.00 - 18.00 sore.
            - Pada hari libur, jumlah peminjaman lebih tersebar merata sepanjang hari, dengan puncak lebih lambat sekitar pukul 12.00 - 15.00 siang.
            """)
            ...
            # Memfilter data untuk hanya menyertakan baris yang tidak memiliki nilai hour
            filtered_df = all_df[all_df['hour'].isna()].copy()
